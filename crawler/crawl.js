...
function extractPlace(html) {
  ...
  return {
    placeName: tag.placeTagTitle || tag.placeTagTitle || '닭꼬치',
...
// ── Supabase에 저장 ────────────────────────────────
async function saveReport({ postId, neighborhoodId, place, postUrl }) {
...
// ── 동네 하나 처리 ────────────────────────────────
async function processNeighborhood(id, crawledIds) {
  ...
  for (const { url, postId } of posts) {
    ...
    await saveReport({ postId, neighborhoodId: id, place, postUrl: url })
    crawledIds.add(postId)
    saved++
    console.log(`  ✓ [${id}] ${place.placeName} | ${place.address} (${place.lat}, ${place.lng})`)
  }
  return saved
}
...
async function main() {
  ...
  await Promise.all(
    ids.map(id =>
      limit(async () => {
        const count = await processNeighborhood(id, crawledIds)
        if (count > 0) {
          total += count
